                           ...
                           const double cutoff)
{
    ...
    std::for_each(rgrid_.begin(), rgrid_.end(), [this, dr](double& r) { r = (&r - rgrid_.data()) * dr; });
                            ...
                            double* dval) const
{
    ...
    ModuleBase::CubicSpline::eval(nr_, rgrid_.data(), tab, dtab, 1, &R, val, dval);
}
...
void TwoCenterTable::two_center_loop(const RadialCollection& bra, const RadialCollection& ket, looped_func f)
{
    for (int l = 0; l <= bra.lmax() + ket.lmax(); ++l)
        for (int l1 = 0; l1 <= bra.lmax(); ++l1)
            for (const NumericalRadial* const* it1 = bra.cbegin(l1); it1 != bra.cend(l1); ++it1)
                for (int l2 = std::abs(l1 - l); l2 <= std::min(ket.lmax(), l + l1); l2 += 2)
                    for (const NumericalRadial* const* it2 = ket.cbegin(l2); it2 != ket.cend(l2); ++it2)
                        (this->*f)(*it1, *it2, l);
}
...
void TwoCenterTable::_tabulate(const NumericalRadial* it1, const NumericalRadial* it2, const int l)
{
    ...
    // Replaced it1->radtab(op_, *it2, l, tab, nr_, rmax_, false) with inline implementation using sbt_

    // Validation checks that were in radtab (assertions)
#ifdef __DEBUG
    assert(it1->nk() > 0 && it1->nk() == it2->nk());
    // We can't easily check std::equal of kgrids efficiently here without looping,
    // but we trust the inputs are compatible as they come from the same cutoff/grid system usually.
#endif

    int nk = it1->nk();
    const double* kgrid = it1->kgrid();
    const double* kv1 = it1->kvalue();
    const double* kv2 = it2->kvalue();

    std::vector<double> fk(nk);
    std::transform(kv1, kv1 + nk, kv2, fk.begin(), std::multiplies<double>());

    double* rgrid_tab = new double[nr_];
    double dr = rmax_ / (nr_ - 1);
    std::for_each(rgrid_tab, rgrid_tab + nr_, [dr, &rgrid_tab](double& r) { r = dr * (int)(&r - rgrid_tab); });

    bool use_radrfft = NumericalRadial::is_fft_compliant(nr_, rgrid_tab, nk, kgrid, 1e-8); // Use explicit tolerance or default? Header has default 1e-8.
    // wait, is_fft_compliant static implementation uses 1e-8 default in header?
    // In radial_function.h: static bool is_fft_compliant(..., const double tol = 1e-8);
    ...
    if (use_radrfft)
    {
        sbt_.radrfft(l, nk, it1->kmax(), fk.data(), tab, it1->pk() + it2->pk() + op_pk);
    }
    else
    {
        sbt_.direct(l, nk, kgrid, fk.data(), nr_, rgrid_tab, tab, it1->pk() + it2->pk() + op_pk);
    }
    ...
    std::for_each(tab, tab + nr_, [pref](double& x) { x *= pref; });
