...
public:
    ...
    // Rule of Zero: Compiler generated copy/move/destructors are sufficient
    // as we use std::vector for resource management.
    ...
    const double* rgrid() const { return rgrid_.data(); }
    const double* kgrid() const { return kgrid_.data(); }
    const double* rvalue() const { return rvalue_.data(); }
    const double* kvalue() const { return kvalue_.data(); }
...
private:
    ...
    std::vector<double> rgrid_;   ///< r-space grid
    std::vector<double> kgrid_;   ///< k-space grid
    ...
    std::vector<double> rvalue_;  ///< r-space value
    std::vector<double> kvalue_;  ///< k-space value
